...
// Intercept and block requests that could trigger sign-in prompts for AI features
const shouldBlockRequest = (req: express.Request): boolean => {
  const url = req.url.toLowerCase();

  // Block GitHub Copilot and AI sign-in
  if (url.includes('github') && (url.includes('copilot') || url.includes('auth') || url.includes('login') || url.includes('signin'))) {
    logger.debug(`Blocked GitHub Copilot auth request: ${req.url}`);
    return true;
  }

  // Block Microsoft AI features
  if (url.includes('microsoft') && (url.includes('ai') || url.includes('auth') || url.includes('login'))) {
    logger.debug(`Blocked Microsoft AI auth request: ${req.url}`);
    return true;
  }

  // Block general AI endpoints
  if (url.includes('/ai/') || url.includes('/intellicode/') || url.includes('/copilot/')) {
    logger.debug(`Blocked AI endpoint: ${req.url}`);
    return true;
  }
...
router.all(/.*/, ensureAuthenticated, ensureVSCodeLoaded, async (req, res) => {
  // Block AI authentication requests
